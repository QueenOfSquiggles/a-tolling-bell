...
func _ready() -> void:
	...
	CtrlDragable.dragable_grabbed.connect(
		func(dragable: CtrlDragable, grab_position: Vector2): _ctrl_drop_zone.activate()
	)
	CtrlDragable.dragable_dropped.connect(
		func(dragable: CtrlDragable, zone: CtrlDropZone, drop_position: Vector2): (
...
func _on_item_drop(
	zone: CtrlDropZone, drop_position: Vector2, ctrl_inventory_item: CtrlInventoryItemRect
) -> void:
...
func get_item_rect(item: InventoryItem) -> Rect2:
	if !is_instance_valid(item):
		return Rect2()
	return Rect2(
		_get_field_position(inventory.get_item_position(item)), _get_item_sprite_size(item)
	)

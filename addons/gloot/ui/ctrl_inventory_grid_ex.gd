...
class PriorityPanel:
	...
	func _ready() -> void:
		...
		mouse_entered.connect(func(): set_style(hover_style))
		mouse_exited.connect(func(): set_style(regular_style))
...
@export_enum("Single", "Multi")
var select_mode: int = CtrlInventoryGridBasic.SelectMode.SELECT_SINGLE:
	set(new_select_mode):
...
func _ready() -> void:
	...
	CtrlDragable.dragable_dropped.connect(
		func(_grabbed_dragable, _zone, _local_drop_position): _fill_background(
			field_style, PriorityPanel.StylePriority.LOW
		)
	)
...
func _highlight_grabbed_item(style: StyleBox):
	...
	var grabbed_item_coords := _ctrl_inventory_grid_basic.get_field_coords(
		global_grabbed_item_pos + (field_dimensions / 2)
	)
